# -*- coding: utf-8 -*-
"""
Created on Mon May 17 22:24:51 2021
...
def genetic_algorithm(cities, adjacency_mat, n_population = 5, n_iter = 20000, selectivity = 0.15, p_cross = 0.5, p_mut = 0.1, print_interval = 100, return_history = False, verbose = False,):
...
if __name__ == "__main__":
    ...
    for V in range(4,21):
        ...
        Vertex.append(V)
    print("DP平均時間:", dynamic_average_times)
    print("GA平均時間:", genetic_average_times)
    print("平均誤差:", average_errors)
    ...
    plt.plot(Vertex, genetic_average_times,c = "g")
    plt.legend(labels=["DP average time", "GA average time"], loc = 'best')
    plt.xlabel("頂點數", fontweight = "bold")
    plt.ylabel("時間", fontweight = "bold")
    plt.title("花費時間", fontsize = 15, fontweight = "bold", y = 1.1)
    plt.show()
    
    plt.figure()
    plt.style.use("ggplot")
    plt.plot(Vertex, average_errors,c = "b")
    plt.legend(labels=["average error"], loc = 'best')
    plt.xlabel("頂點數", fontweight = "bold")
    plt.ylabel("誤差", fontweight = "bold")
    plt.title("平均誤差", fontsize = 15, fontweight = "bold", y = 1.1)
